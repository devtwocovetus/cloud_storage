class AssetLocationModel {
  ...
  List<AssetLocation>? data;

  AssetLocationModel({this.status, this.message, this.pagination, this.data});

  AssetLocationModel.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    ...
    if (json['data'] != null) {
      data = <AssetLocation>[];
      json['data'].forEach((v) {
        data!.add(AssetLocation.fromJson(v));
      });
    }
  }
...
class AssetLocation {
  ...
  AssetLocation.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    name = json['name'];
    email = json['email'];

class TransactionLogDetailTrInModel {
  int? status;
  String? message;
  List<ItemTrIn>? data;

  TransactionLogDetailTrInModel({this.status, this.message, this.data});

  TransactionLogDetailTrInModel.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    message = json['message'];
    if (json['data'] != null) {
      data = <ItemTrIn>[];
      json['data'].forEach((v) {
        data!.add(ItemTrIn.fromJson(v));
      });
    }
  }
...
class ItemTrIn {
  ...
  ItemTrIn.fromJson(Map<String, dynamic> json) {
    id = json['id'];
